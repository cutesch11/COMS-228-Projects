...
public class Video implements Comparable<Video> {
	 ...
	 *             if copies <= 0
	 */
	Video(String film, int n) throws IllegalArgumentException {
		this.film = film;
		numCopies = n;
		numRentedCopies = 0;
	}
	 ...
	 */
	public void rentCopies(int n) throws IllegalArgumentException,
			AllCopiesRentedOutException {
		if (n <= 0) {
			throw new IllegalArgumentException();
		}

		if (numCopies == numRentedCopies) {
	...
	/**
	 * Updates numRentedCopies. If n > numRentedCopies, set numRentedCopies to
	 * zero.
	 * 
	 * @param n
	 * @throws IllegalArgumentException
	 ...
	 * brackets is replaced with the value of the variable. For example, if a
	 * Video object has its private instance variables take on the values below:
	 * 
	...
	public String toString() {
		String s = this.film + " " + "(" + numCopies + ":" + numRentedCopies
				+ ")";
		return s;
	}
}
