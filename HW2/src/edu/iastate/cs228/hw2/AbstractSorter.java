...
public abstract class AbstractSorter {
	...
	protected AbstractSorter(Point[] pts) throws IllegalArgumentException {
		...
		for (int i = 1; i < points.length; ++i) // Finds and sets lowestPoint
		{
	...
	protected AbstractSorter(String inputFileName)
		...
		while (t.hasNextLine() && t.hasNextInt()) {
			nums[i] = t.nextInt();
			t.nextLine();
			++i;
		}
		t.close();
		points = new Point[elements / 2];

		for (int x = 0, y = 0; x < (elements / 2); ++x, y += 2) {
			points[x] = new Point(nums[y], nums[y + 1]);
		}

		lowestPoint = points[0];

		for (int j = 1; j < points.length; ++j) // Finds and sets lowestPoint
		{
			if (points[j].getY() < lowestPoint.getY()
					|| (points[j].getY() == lowestPoint.getY() && points[j]
							.compareTo(lowestPoint) == -1)) {
	...
	public String stats() {
		String statistics = "";
		if (this.outputFileName == "insert.txt") {
			statistics += "insertion sort" + "   " + points.length + "     "
					+ sortingTime;
		}

		if (this.outputFileName == "select.txt") {
			statistics += "selection sort" + "   " + points.length + "     "
					+ sortingTime;
		}

		if (this.outputFileName == "quick.txt") {
			statistics += "quicksort" + "        " + points.length + "     "
					+ sortingTime;
		}

		if (this.outputFileName == "merge.txt") {
			statistics += "mergesort" + "        " + points.length + "     "
					+ sortingTime;
		}
	...
	public void writePointsToFile() throws FileNotFoundException {
		...
		if (sortByAngle == false) {
			for (int i = 0; i <= points.length - 1; ++i) {
				write.print(points[i].getX() + " " + points[i].getY());
				write.println();
			}
			write.close();
		}
			...
			write.println(points[0].getX() + " " + points[0].getY());
			for (int x = 1; x <= points.length - 1; ++x) {
				write.print(points[x].getX() + " " + points[x].getY() + " "
						+ points[0].getX() + " " + points[0].getY() + " "
						+ points[x].getX() + " " + points[x].getY());
				write.println();
			}
		}
		write.close();
