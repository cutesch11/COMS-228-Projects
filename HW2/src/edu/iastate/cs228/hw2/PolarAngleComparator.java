...
public class PolarAngleComparator implements Comparator<Point> {
	...
	public int compare(Point p1, Point p2) {
		// DOUBLE CHECK LATER
		if (p1.equals(p2) == true && p1.equals(referencePoint) == false) {
			return 0;
		}

		else if ((p1.equals(referencePoint) && p2.equals(referencePoint) == false)
				|| (p1.equals(referencePoint) == false
						&& p2.equals(referencePoint) == false && comparePolarAngle(
						p1, p2) == -1)
				|| (p1.equals(referencePoint) == false
						&& p2.equals(referencePoint) == false
	...
	private int crossProduct(Point p1, Point p2) {
		Point vec1 = new Point((p1.getX() - referencePoint.getX()),
				(p1.getY() - referencePoint.getY()));
		Point vec2 = new Point((p2.getX() - referencePoint.getX()),
				(p2.getY() - referencePoint.getY()));
		int crossProd = ((vec1.getX() * vec2.getY()) - (vec2.getX() * vec1
				.getY()));
		return crossProd;
	}
	...
	private int dotProduct(Point p1, Point p2) {
		...
		Point vec2 = new Point((p2.getX() - referencePoint.getX()),
				(p2.getY() - referencePoint.getY()));
		int dotProd = (vec1.getX() * vec2.getX()) + (vec1.getY() * vec2.getY());
		return dotProd;
	}
}
